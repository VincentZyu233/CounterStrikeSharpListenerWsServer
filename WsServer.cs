...
public class WsServer
{
    ...
    public WsServer(ILogger logger)
    {
        _logger = logger;
    }
    ...
    public async Task StartAsync(string host, int port, string token)
    {
        ...
        {
            _logger.LogError($"[WsServer] Failed to start HttpListener: {ex.Message}");
            return;
        }
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                while (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    try
                    {
                        ...
                        if (completed != contextTask || cancellationToken.IsCancellationRequested)
                            break;
                    ...
                    {
                        _logger.LogError($"[WsServer] Accept error: {ex.Message}");
                        break;
                    }
                }
            }
            ...
            {
                _logger.LogInformation("[WsServer] Accept loop stopped");
            }
        }, cancellationToken);
    }
    ...
    private async Task ReceiveLoopAsync(WebSocket ws, CancellationToken ct)
    {
        ...
        try
        {
            while (ws.State == WebSocketState.Open && !ct.IsCancellationRequested)
            {
                ...
                {
                    _logger.LogInformation("[WsServer] Client sent close frame");
                    break;
                }
        ...
        {
            _logger.LogDebug($"[WsServer] WebSocket error: {ex.Message}");
        }
        catch (OperationCanceledException)
        {
        ...
        {
            _logger.LogError($"[WsServer] ReceiveLoop error: {ex.Message}");
        }
        finally
        {
            lock (_lock)
